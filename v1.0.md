...
[ProviderBootstrap] 服务未找到: com.malinghan.marpc.demo.HelloService

skeleton map 里没有注册 HelloService。根因在 ProviderBootstrap.start() 的扫描逻辑：

// ProviderBootstrap.java:33
for (Class<?> iface : bean.getClass().getInterfaces()) {

HelloServiceImpl 被 Spring 用 CGLIB 代理包装后，bean.getClass() 返回的是 HelloServiceImpl$$SpringCGLIB$$0 这类代理类，而不是 HelloServiceImpl。代理类的 getInterfaces() 可能返回空，或者返回的是代理相关接口，导致 HelloService
没被注册进去。

修复方式：用 AopUtils.getTargetClass() 或者向上遍历父类来获取真实类型：
...
Class<?> targetClass = AopUtils.getTargetClass(bean); // 获取真实实现类
